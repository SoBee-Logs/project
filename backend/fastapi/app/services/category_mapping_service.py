...
## 분류 규칙
1. 카드사분류는 참고만 하세요. 카드사가 잘못 분류한 경우가 많습니다.
2. 가맹점명을 우선으로 판단하세요.
3. PG 계열(결제대행, 인터넷P/G 등)은 업종이 아닙니다. 반드시 가맹점명으로 판단하세요.
4. 법인명 패턴(주식회사 OO, (주)OO) → 괄호 안 실제 상호명으로 판단하세요.
5. 특수문자 깨진 경우(편+의+점, 화+++원) → 원래 단어로 복원해서 판단하세요.
...
7. 아래는 자주 틀리는 케이스입니다. 반드시 지키세요:
   ...
   - ANTHROPIC, PBC 등 소프트웨어/AI 구독 → 문화/여가(7)
   - 구글페이먼트코리 → 문화/여가(7)
