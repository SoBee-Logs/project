...
## 최우선 규칙 — 간편결제 (반드시 먼저 확인)
결제 장소가 카카오페이, 네이버페이, 삼성페이, 토스, 페이코, 애플페이 등 간편결제이면:
- 위치, 카테고리, 가게명은 완전히 무시해
- 시간 차이 2시간 이내 AND 금액이 추정 금액의 0.5배~2배 범위이면 → 즉시 매핑해
- 더치페이 의심 시 추정 금액의 1/4~1/2 범위도 허용
- 위 시간·금액 조건을 충족하면 다른 규칙은 읽지 말고 바로 결과 반환해
...
2. 품목/카테고리와 결제 장소 연관성
   품목, 카테고리를 결제 장소명과 비교해.
   payment_category는 카드사 오분류가 많으니 무시하고 품목 위주로 판단해.
